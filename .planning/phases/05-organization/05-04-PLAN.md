...
<task type="auto">
  <name>Task 3: Create tag picker component and integrate into pages</name>
  <files>templates/partials/tag_picker.templ, templates/pages/admin/document_detail.templ, templates/pages/admin/documents.templ</files>
  <action>
...
- Add small tag display in document list cards
- Optionally: Add inline tag picker on hover/click (or link to detail page)

Update internal/handler/documents.go:
- Modify DocumentDetailPage to also fetch and pass document tags
  </action>
  <verify>Run `make dev`, navigate to document detail page. Tag picker should appear. Test adding/removing tags. Check document list shows tags.</verify>
  <done>Tag picker integrated into document detail page. Can search, add, remove, and create tags inline. Document list shows assigned tags.</done>
</task>
...
9. Tags persist after page refresh (verify database)
</verification>

<success_criteria>
- Tag picker component is reusable (works in multiple contexts)
- Search filters existing tags with 300ms debounce
- Can add existing tags to documents
- Can create and add new tags inline
- Can remove tags from documents
- Document detail page shows current tags
- Document list shows tags on cards
