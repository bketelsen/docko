...
<task type="auto">
  <name>Task 1: Create migration and correspondent SQL queries with document counts</name>
  <files>internal/database/migrations/006_correspondent_notes.sql, sqlc/queries/correspondents.sql</files>
  <action>
First, create the migration file for the notes column:

Create migration file `internal/database/migrations/006_correspondent_notes.sql`:
```sql
-- +goose Up
...
Then create sqlc queries for correspondent CRUD operations in `sqlc/queries/correspondents.sql`:
...
- correspondents: id (UUID), name (VARCHAR 255), notes (TEXT, nullable), created_at
  ...
  <verify>Run `make migrate` to apply the migration. Run `make generate` and check `./tmp/air-combined.log` for sqlc/migration errors. Verify internal/database/sqlc/correspondents.sql.go is generated.</verify>
  <done>Migration applied, all 6 correspondent queries compile without errors, and generated Go code exists.</done>
</task>
...
<task type="auto">
  <name>Task 3: Create correspondent management page template with modal dialog and sidebar link</name>
...
3. "Add Correspondent" button that opens a modal dialog:
...
4. Create/Edit Correspondent modal dialog:
   ...
   - Form uses hx-post="/correspondents" hx-target="#correspondent-list" hx-swap="beforeend" for create
   - For edit: hx-post="/correspondents/:id" with hx-swap="outerHTML" targeting the card
   - Close modal on success: use HX-Trigger response header or hx-on::after-request

5. Correspondent list (#correspondent-list div):
...
6. CorrespondentCard component:
   ...
   - Edit button (opens modal dialog pre-filled with correspondent data)
...
- Use SVG user icon
  ...
  <verify>Run `make dev`, navigate to http://localhost:3000/correspondents (after login). Page should render with Add Correspondent button that opens modal. Check `./tmp/air-combined.log` for template errors.</verify>
  <done>Correspondent management page renders correctly with modal dialog for create/edit operations. Sidebar shows Correspondents link.</done>
</task>

</tasks>

<verification>
1. Navigate to /correspondents page - shows correspondent management UI with "Add Correspondent" button
2. Click "Add Correspondent" - modal dialog opens with name input and notes textarea
3. Create a correspondent "Acme Corp" with notes "Vendor for office supplies" - modal closes, appears in list
4. Create another correspondent "John Smith" with no notes - both visible
5. Click edit on "Acme Corp" - modal opens pre-filled with current values
6. Add more notes, save - modal closes, correspondent updates in list
7. Delete "John Smith" (confirm dialog shows "0 documents") - correspondent removed
8. Sidebar navigation includes Correspondents link with correct icon
</verification>

<success_criteria>
- Correspondent management page accessible at /correspondents
- Modal dialog opens for creating new correspondents
- Modal dialog opens for editing existing correspondents (pre-filled)
- Can create correspondents with name and optional notes via modal
- Can edit existing correspondent name and notes via modal
